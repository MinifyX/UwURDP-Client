...
export interface SetupApi {
  ...
  /** macOS and Linux: switch to uninstalling the installed UwURDP. */
...
function previewApi(): SetupApi {
  ...
  const dir = 'C:\\Users\\Nyu\\AppData\\Local\\Programs\\UwURDP';
  const pretend = async (steps: Step[]) => {
    ...
    if (params.has('fail')) throw `Couldn't write ${dir}\\UwURDP.exe`;
  };
  return {
    info: async () => ({
    ...
    pickFolder: async () => 'D:\\Apps\\UwURDP',
    closeApp: async () => {
      running = false;
    },
